...
# Define color map for entity types
ENTITY_COLORS = {
    'PER': (255, 0, 0),      # Red for Person
    'LOC': (0, 255, 0),      # Green for Location
    'ORG': (0, 0, 255),      # Blue for Organization
    # Add more as needed
}

def get_entity_color(label):
    return ENTITY_COLORS.get(label, (0, 255, 255))  # Default: Yellow

def overlay_entities_on_image(image, ocr_results, ner_results):
    output_image = image.copy()
    word_to_entity = {entity['word']: entity['entity_group'] for entity in ner_results}
    for (bbox, text, prob) in ocr_results:
        # Clean the text to remove '?'
        clean = text.replace('?', '').strip()
        if not clean:
            continue  # Skip empty results after cleaning
        points = np.array(bbox, np.int32).reshape((-1, 1, 2))
        entity_label = word_to_entity.get(text, None)
        color = get_entity_color(entity_label) if entity_label else (0, 255, 0)
        cv2.polylines(output_image, [points], True, color, 2)
        label = f"{clean}"
        if entity_label:
            label += f" ({entity_label})"
        # Place text above the first point of the box
        cv2.putText(output_image, label, (int(bbox[0][0]), int(bbox[0][1]) - 10),
